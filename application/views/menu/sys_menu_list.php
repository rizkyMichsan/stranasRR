        ...
        $(document).ready(function(){
            ...
            $('.create').on('click',function(){
               ...
               $("#icon_form").addClass('hidden');
                $('#Sys_menu_form').find('.form-line').removeClass('focused');$('#menu_name').val('');
                $('#icon').val('');
                $('#link').val('');$('#parent').val('');$('#stats').val('');$('#stats').selectpicker('refresh');
                $('#order').val('');$('#Sys_menu_form').find('#menu_form').attr('action','<?=base_url()?>menu/create');
            });
        ...
        $('.table-responsive').on('click','#update',function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                            ...
                            $('#icon').val(data['icon']);
                            $('#link').val(data['link']);
                            $('#stats').val(data['stats']);$('#stats').selectpicker('refresh');
            ...
            function getParent(iu){
                ...
                var ids='';
                if(id=="mm"){ids=0;}
                else if(id=="sm"){ids='mm';}
                else if(id=="ssm"){ids='sm';}
                $.ajax({
                    type : "GET",
